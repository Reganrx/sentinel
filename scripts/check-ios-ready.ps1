...
if ($sourceBuild -lt 27) { throw "The source build number $sourceBuild must be 27 or higher for the next TestFlight upload." }
...
Write-Host "Source version: 1.2.1 ($sourceBuild)"
Write-Host "The next TestFlight upload will use build $sourceBuild; increase it again before every later upload."
