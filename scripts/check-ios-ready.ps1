...
if ($sourceBuild -lt 14) { throw "The source build number $sourceBuild must be 14 or higher for the next TestFlight upload." }
...
Write-Host "Source version: 1.2.1 ($sourceBuild); latest uploaded TestFlight build: 13"
Write-Host 'Next TestFlight upload must use build 14 or higher.'
