...
private struct SentinelPageView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if page == .chat {
                ...
                ScrollView { VStack(alignment: .leading, spacing: 16) { Header(page: page); content }.padding() }
    ...
    private var batteryColor: Color { guard let level = app.localSystem.batteryLevel else { return .secondary }; return level > 20 ? .cyan : .orange }
...
private struct SentinelWeatherDashboard: View {
    ...
    private var radar: some View { Card(title: "WEATHER RADAR", symbol: "map.fill") {
        ...
        if let frame {
            ...
            Slider(value: Binding(get: { Double(radarFrameIndex) }, set: { radarFrameIndex = Int($0.rounded()); stopRadarPlayback() }), in: 0...Double(max(0, frames.count - 1)), step: 1).tint(.cyan).disabled(frames.count < 2)
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 12) {
                        if app.chatMessages.isEmpty {
                            VStack(spacing: 12) {
                                Image(systemName: "sparkles").font(.system(size: 30)).foregroundStyle(.cyan)
    ...
    private var composer: some View {
        VStack(spacing: 8) {
            ...
            HStack(alignment: .bottom, spacing: 10) {
                Menu { Button { importingChatAttachment = true } label: { Label("Choose file", systemImage: "folder") }; Button { cameraPresented = true } label: { Label("Take photo", systemImage: "camera") }; PhotosPicker(selection: $selectedPhoto, matching: .images) { Label("Photo library", systemImage: "photo.on.rectangle") } } label: { Image(systemName: "plus.circle.fill").font(.system(size: 31)) }.tint(.cyan).disabled(app.isSendingChat).accessibilityLabel("Add attachment")
                ...
                Button { app.toggleVoiceChat() } label: { Image(systemName: app.voiceStatus == "Listening…" ? "waveform.circle.fill" : "mic.circle.fill").font(.system(size: 30)) }.buttonStyle(.plain).foregroundStyle(.cyan).disabled(app.isSendingChat)
                Button { live.open(conversationId: app.conversationID, enabledServices: app.enabledMobileServices.sorted()) } label: { Image(systemName: "waveform.badge.mic").font(.system(size: 25)) }.buttonStyle(.plain).foregroundStyle(.cyan).accessibilityLabel("Talk with Sentinel")
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack { ForEach(app.chatCards) { card in
                        VStack(alignment: .leading, spacing: 5) {
                            Label(card.type.capitalized, systemImage: icon(for: card.type)).font(.caption.bold()).foregroundStyle(.cyan)
                            ...
                            if let value = card.value, !value.isEmpty { Text(value).font(.title3.bold()).foregroundStyle(.cyan) }
                        ...
                        }.frame(width: 205, alignment: .leading).padding(12).background(Color.white.opacity(0.08), in: RoundedRectangle(cornerRadius: 16)).overlay(RoundedRectangle(cornerRadius: 16).stroke(Color.cyan.opacity(0.2)))
                    } }
                }
            }
            if !app.chatActions.isEmpty { HStack { ForEach(app.chatActions) { action in Button(action.label ?? "Open") { app.runChatAction(action) }.buttonStyle(.bordered).tint(.cyan) } } }
        }.padding(.vertical, 4)
    }
